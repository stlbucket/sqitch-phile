-- Deploy auth:function/authenticate to pg
-- requires: custom-type/jwt-token
-- requires: structure/app_user

BEGIN;

  CREATE function auth.build_app_user(
    _app_tenant_id uuid,
    _username text,
    _password text,
  ...
  --||--
  comment on function auth.build_app_user(
    uuid,
    text,
    text,
    auth.permission_key
  ...
  --||--
  GRANT execute ON FUNCTION auth.build_app_user(
    uuid,
    text,
    text,
    auth.permission_key
